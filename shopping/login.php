...
// admin login		
		...
		$q = "SELECT * FROM t_admin_mst WHERE adm_email='$emailid' AND adm_password='$password'";
...
// user login	
		...
		$que="SELECT user_id from users WHERE email='".$emailid."'" ;
		$result=mysqli_query($conn,$que);
		$row= mysqli_fetch_array($result);
		$_SESSION['user_id']=$row['user_id'];
        header("location:userhome.php?user_id=$row[user_id]");
        } else {  
            // Registration Failed  
